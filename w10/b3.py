...
a, b, c = 33, 120, 40
name = input()
conn = psycopg2.connect(host='localhost', database='postgres', user='postgres', password='alma45884')
cursor = conn.cursor()
sql = "select exists(select 1 from users_score where username=%s)"
...
if(ans[0]):
    ...
    sql = "insert into users_score (username, score) values(%s, %s);"
